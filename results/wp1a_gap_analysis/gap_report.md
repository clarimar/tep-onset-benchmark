# EERS Stage 27 — Project-to-Manuscript Gap Analysis
...
- **Project evidence coverage:** 100.0%
- **Manuscript evidence coverage:** 27.8%
- **Incorporation rate:** 27.8%
- **Project-to-manuscript gap:** 72.2 percentage points
- **Produced but not incorporated:** 13
- **Mentioned without located project evidence:** 0
- **Missing from both project and manuscript:** 0

## Produced but not incorporated
...
- Action: Report the Friedman test, test statistic, p-value, number of trajectories, and interpretation.
...
- Manuscript evidence: not detected
- Target section: **Results / Statistical analysis**
- Action: Add the Nemenyi post-hoc procedure, corrected pairwise results, critical difference, and significant comparison count.
...
- Action: Include or cite the Critical Difference diagram and explain the groups of statistically indistinguishable models.
...
- Action: Explain how validation guided model selection and how the test partition remained isolated until final evaluation.
...
- Action: Summarize the data-quality checks: schema consistency, missing values, duplicates, trajectory integrity, and class coverage.
...
- Action: Report the per-class sampling limits, resulting sample counts, and rationale for balanced macro-level comparison.
...
- Action: Quantify the absolute and relative performance gaps between the best ensemble, simpler trees, and linear baselines.
...
- Action: Discuss the trade-off between predictive performance, training time, inference cost, and model complexity.
...
- Action: State that the reproducibility package contains tables, figures, metadata, environment information, and checksums.
...
- Action: Add a Code Availability Statement. Use a repository URL when public, or state that code will be released upon acceptance.
...
- Action: Add a formal statement identifying the public TEP data source and the exact files used.
...
- Action: Report Python and core library versions used in the benchmark.
...
- Action: Report random_state=42 and any deterministic execution settings.


## Already incorporated

- **Detailed class/fault-level analysis** — manuscript evidence: class-level, fault-level
- **Global benchmark comparison** — manuscript evidence: histgradientboosting, random forest, extra trees, logistic regression, linear svm
- **Run-level or trajectory-level splitting** — manuscript evidence: run-level, independent runs
- **Primary metric justification** — manuscript evidence: primary metric, f1-score
- **Hardware specification** — manuscript evidence: ram

## Interpretation

The project contains more scientific evidence than is currently visible in the manuscript. The highest-priority revisions are those for which robust artefacts already exist but the associated methodological or statistical evidence is absent from the text.
