...
body = body.replace("width=\\columnwidth", "width=0.62\\textwidth")
...
# The repository URL and the Zenodo DOI both identify the authors. For
# double-anonymized review they are replaced by an anonymous mirror
# (anonymous.4open.science), which lets reviewers inspect the code without
# revealing authorship. The real URL and DOI return in the accepted version.
import re as _re
_dep = _re.search(
    r"are deposited at\s*\n\\url\{[^}]*\} and archived under\s*\n"
    r"(?:\\TBD\{[^}]*\}|[0-9./a-zA-Z_-]+)\.",
    body)
if _dep:
    if args.anon_url:
        _repl = ("are available for review at an anonymized mirror,\n"
...
%% Compact the reference list: 42 entries at 12 pt otherwise run long.
\usepackage{etoolbox}
